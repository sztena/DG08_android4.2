...
/* Version Information */
#define DRIVER_VERSION "v0.9.5 (2013/01/22)"
...
**
*/
static int get_registers(rtl8152_t *tp, u16 value, u16 index, u16 size, void *data)
{
...
static int set_registers(rtl8152_t *tp, u16 value, u16 index, u16 size, void *data)
{
...
static int pla_ocp_read(rtl8152_t *tp, u16 index, u16 size, void *data)
{
	...
	if (test_bit(RTL8152_UNLINK, &tp->flags))
		return -ENODEV;
...
static int pla_ocp_write(rtl8152_t *tp, u16 index, u16 byteen, u16 size, void *data)
{
	int	ret;
...
static int usb_ocp_write(rtl8152_t *tp, u16 index, u16 byteen, u16 size, void *data)
{
	int	ret;
	u16	byteen_start, byteen_end, byen;
	u16	limit = 512;
...
static void read_bulk_callback(struct urb *urb)
{
	...
	case 0:
		break;
	case -ESHUTDOWN:
		set_bit(RTL8152_UNLINK, &tp->flags);
	...
	skb = netdev_alloc_skb_ip_align(netdev, RTL8152_MTU);
	if (!skb) {
		goto goon;
	}
...
//	count = (skb->len < 60) ? 60 : skb->len;
	count = skb->len;
	if (skb_headroom(skb) < sizeof(*tx_desc)) {
		tx_skb = dev_alloc_skb(count + sizeof(*tx_desc));
		tx_desc = (TxDesc *)skb_put(tx_skb, sizeof(*tx_desc));
		memcpy(skb_put(tx_skb, skb->len), skb->data, skb->len);
		dev_kfree_skb_any(skb);
	} else {
		tx_skb = skb;
		tx_desc = (TxDesc *)skb_push(tx_skb, sizeof(*tx_desc));
	}
	memset(tx_desc, 0, sizeof(*tx_desc));
	tx_desc->opts1 = cpu_to_le32((count & 0xffff) | FirstFrag | LastFrag);
	tp->tx_skb = tx_skb;
	usb_fill_bulk_urb(tp->tx_urb, tp->udev, usb_sndbulkpipe(tp->udev, 2),
			  tx_skb->data, tx_skb->len,
...
static int rtl8152_set_speed(struct net_device *netdev, u8 autoneg, u16 speed, u8 duplex)
{
	rtl8152_t *tp = netdev_priv(netdev);
	u16 bmcr, anar;
	int ret = 0;
...
{
	return rtl8152_set_speed(dev, cmd->autoneg, cmd->speed, cmd->duplex);
}
			 ...
			 const struct usb_device_id *id)
{
	...
	printk("\33[32m [MARCO][%s][%d]---RTL8152 probe---\33[0m\n", __func__, __LINE__); 
